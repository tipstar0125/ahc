...
fn main() {
    ...
    'a: for sep in 0..input.N {
        ...
        let mut bin_width = make_partition(&input, sep, &mut rng);
        let (mut ok, mut assignments) = solve_bin_packing(&bin_width, &input);
        let mut score = calc_score(&input, &bin_width, &assignments);
        // 各仕切り毎の探索時間の上限
        while get_time() - elapsed < 0.3 {
            ...
            let id1 = rng.gen_range(0..bin_width.len());
            let id2 = rng.gen_range(0..bin_width.len());
            if id1 == id2 {
                continue;
            }
            let width = bin_width[id1];
            if width / 4 <= 1 {
                continue;
            }
            let diff = rng.gen_range(1..width / 4);
            bin_width[id1] -= diff;
            bin_width[id2] += diff;
            let (next_ok, next_assignments) = solve_bin_packing(&bin_width, &input);
            let next_score = calc_score(&input, &bin_width, &next_assignments);
            if next_ok || next_score <= score {
                score = next_score;
                ok = next_ok;
    ...
    // 最大の仕切り数で時間いっぱい探索
    let sep = best_bin_width.len() - 1;
    eprintln!("Sep: {}", sep);
    let mut bin_width = if sep == 0 {
        make_partition(&input, 1, &mut rng)
    } else {
        best_bin_width.clone()
    };
    while get_time() < 2.9 {
        let id1 = rng.gen_range(0..bin_width.len());
        let id2 = rng.gen_range(0..bin_width.len());
