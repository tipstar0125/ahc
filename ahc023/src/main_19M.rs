    ...
    fn can_harvest_all(&self, y: usize, x: usize) -> bool {
        ...
        while let Some((now_y, now_x)) = Q.pop_front() {
            for &(next_y, next_x) in &self.G[now_y][now_x] {
                ...
                if !visited[next_y][next_x] && self.crop[next_y][next_x] == (0, 0) {
                    visited[next_y][next_x] = true;
                    Q.push_back((next_y, next_x));
                }
            }
        ...
        // for i in 0..H {
        //     for j in 0..W {
        //         if self.crop[i][j] == (0, 0) {
        //             continue;
        //         }
        //         if !self.can_harvest(i, j, self.crop[i][j].1, &set) {
        //             ok = false;
        //         }
        //     }
        // }
...
impl Solver {
    fn solve(&mut self) {
        ...
        // let mut c = 0;
        for &(s, d, k) in &SDK {
            // for &(s, d, k) in &SDK[0..min!(2500, SDK.len())].to_vec() {
            // c += 1;
            // eprintln!("{}", c);
            let can_plant_list = state.can_plant_section_list();
            let can_plant_list2 = can_plant_list[0..min!(160, can_plant_list.len())].to_vec();
