...
impl Solver {
    fn solve(&mut self) {
        ...
        for &(s, d, k) in &SDK {
            ...
            // for &(s, d, k) in &SDK[0..min!(3000, SDK.len())].to_vec() {
            // c += 1;
            // eprintln!("{}", c);
            let can_plant_list = state.can_plant_section_list();
            let can_plant_list2 = can_plant_list[0..min!(150, can_plant_list.len())].to_vec();

            let mut cnt = 0;

            while cnt < 1 {
                // let idx = rnd::gen_range(0, can_plant_list.len());
