...
impl State {
    ...
    fn action(&mut self, mut py: usize, mut px: usize, bombs: &[Bomb], used: &[bool]) {
        // buy
        for i in 0..used.len() {
            if used[i] {
                self.actions.push((2, i + 1));
                self.b += 1;
                self.cost += bombs[i].C;
            }
        }
        ...
        for i in 0..used.len() {
            if used[i] {
                self.actions.push((3, i + 1));
                self.destruct_A(py, px, &bombs[i]);
                self.b -= 1;
            }
        }
...
impl Solver {
    fn solve(&mut self) {
        ...
        let (sy, sx, used) = state.init(&bombs);
        // eprintln!("{} {}", sy, sx);
        // eprintln!("{:?}", used);
        state.get_explosion_positions(&bombs);
        // eprintln!("{}", state.explosion_positions.len());
        // eprintln!("{:?}", state.explosion_positions);
        state.action(sy, sx, &bombs, &used);
        state.output();
    }
}
