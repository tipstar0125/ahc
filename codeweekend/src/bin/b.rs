...
fn solve(filename: &str) {
    ...
    let mut monster_attack: BTreeMap<usize, usize> = BTreeMap::new();
    for m in input.monsters.iter() {
        *monster_attack.entry(m.attack).or_default() += 1;
    }
    println!("{:?}", monster_attack);
    ...
    while t < input.num_turns {
        ...
        while !state.is_done(&input) {
            ...
            print!("turn: {}", state.ans.len());
        }
        if state.gold > best_score {
            best_score = state.gold;
            best_ans = state.ans;
        }
        println!("t: {}, Score: {}", t, state.gold);
        // t += 10;
        break;
    }
...
impl State {
    ...
    fn calc_gold(&self, gold: usize) -> usize {
        ((gold as f64) * 1000.0 / (1000.0 + self.fatigue as f64)) as usize
    }
    fn calc_gold_input(&self, gold: usize, fatigue: usize) -> usize {
        ((gold as f64) * 1000.0 / (1000.0 + fatigue as f64)) as usize
    }
    fn advance(
        &mut self,
        target_id: usize,
        ...
        for i in 0..actions.len() - 1 {
            let (t, x, y) = actions[i];
            if t == 0 {
                self.fatigue += self.calc_monster_attack(x, y, input, !0);
            } else {
                self.fatigue += self.calc_monster_attack(nx, ny, input, !0);
            }
        }
        ...
        self.gold += self.calc_gold(input.monsters[target_id].gold);
        while self.level_up(input) {}
        self.fatigue += self.calc_monster_attack(self.x, self.y, input, !0);
    }
    fn get_next_target(
        &self,
        input: &Input,
        ...
        for id in 0..input.monsters.len() {
            ...
            if input.monsters[id].attack > 45 {
                continue;
            }
            let mut x = self.x;
            let mut y = self.y;
            ...
            while diff2 > self.range2 {
                ...
                actions[id].push((0, x, y));
                fatigue += self.calc_monster_attack(x, y, input, !0);
            ...
            fatigue -= self.calc_monster_attack(x, y, input, !0);
            fatigue += self.calc_monster_attack(x, y, input, id);
            ...
            let score = if self.ans.len() <= t {
                ...
                self.calc_gold_input(input.monsters[id].gold, fatigue)
            };
    ...
    fn calc_monster_attack(&self, x: usize, y: usize, input: &Input, dead_id: usize) -> usize {
        let mut fatigue = 0;
        for id in 0..input.monsters.len() {
            if self.monsters_status[id] {
                continue;
            ...
            let mx = input.monsters[id].x;
            let my = input.monsters[id].y;
            let m_range = input.monsters[id].range;
            let m_attack = input.monsters[id].attack;
            let diff2 = calc_diff2(x, y, mx, my);
            if diff2 <= m_range * m_range {
                fatigue += m_attack;
            }
        }
        fatigue
    }
    fn advance_monster_attack(&mut self, input: &Input) {
        let fatigue = self.calc_monster_attack(self.x, self.y, input, !0);
