...
fn main() {
    ...
    'outer: while cnt < mino_num {
        ...
        let out = query(&[now], &ans, eps, &es, &mut turn, &mut cost) as usize;
        if out == 0 {
            mp[now] = Some(0);
            continue;
        }
        ...
        while let Some(u) = Q.pop_front() {
            for adj in ADJ {
                ...
                let out = query(&[v], &ans, eps, &es, &mut turn, &mut cost) as usize;
                if out == 0 {
                    mp[v] = Some(0);
                } else {
                    mp[v] = Some(out);
    ...
    for i in 0..N {
        for j in 0..N {
            let now = Coord::new(i, j);
            if ans[now] > 0 && mp[now] == Some(0) {
                ac = false;
            }
            if ans[now] > 0 && mp[now].is_none() {
                ac = false;
            }
            if ans[now] == 0 && mp[now].is_some() && mp[now].unwrap() > 0 {
                ac = false;
            }
        }
    }
...
fn vis2(mp: &DynamicMap2d<Option<usize>>, ans: &DynamicMap2d<i32>, N: usize) {
    let size = 600.0;
    let mut doc = doc(size, size);
    doc = doc.add(Style::new(format!(
        "text {{text-anchor: middle; dominant-baseline: central; font-size: {}}}",
