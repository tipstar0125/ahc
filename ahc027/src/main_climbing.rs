...
impl Solver {
    fn solve(&mut self) {
        ...
        while now_state.actions.len() <= TURN && !time_keeper.isTimeOver() {
            cnt += 1;

            if !now_state.is_visited() {
                if cnt % 30 != 5 {
                    let mut candidate = vec![];
                    ...
                    now_state.action((y, x), &G, &d);
                }
            } else {
                let mut next_state = now_state.clone();
                let mut candidate = vec![];
                let (y, x) = next_state.now;
                for &(ny, nx) in &nears[y][x] {
                    candidate.push((next_state.dirty[ny][nx], ny, nx));
                }
                candidate.sort();
                candidate.reverse();
                let (_, ny, nx) = candidate[0];
                next_state.action((ny, nx), &G, &d);
                let mut next_finish_state = next_state.clone();
                next_finish_state.action((0, 0), &G, &d);
                let score = next_finish_state.calc_score(&d);
                if best_score > score && next_finish_state.actions.len() <= TURN {
                    best_score = score;
                    best_state = next_finish_state;
                    now_state = next_state;
                } else {
                    let (ny, nx) = now_state.get_max_position();
                    now_state.action((ny, nx), &G, &d);
                    let mut finish_state = next_state.clone();
                    finish_state.action((0, 0), &G, &d);
                    let score = finish_state.calc_score(&d);
                    if best_score > score && finish_state.actions.len() <= TURN {
                        best_score = score;
                        best_state = finish_state;
                    }
                }
            }
        }
