...
impl State {
    ...
    fn random_walk(&mut self, y: &mut usize, x: &mut usize, num: usize) {
        let mut visited = BTreeSet::new();
        for _ in 0..num {
            visited.insert((*y, *x));
            let L = self.G[*y][*x].len();
            let mut ny = *y;
            let mut nx = *x;
            let mut try_cnt = 0;
            while visited.contains(&(ny, nx)) {
                let r = rnd::gen_range(0, L);
                let pos = self.G[*y][*x][r];
                ny = pos.0;
                nx = pos.1;
                try_cnt += 1;
                if try_cnt >= 4 {
            ...
            self.last_visited[*y][*x] = self.turn;
            let dir = self.encode_dir(*y, *x, ny, nx);
            self.actions.push(dir);
            self.turn += 1;
            *y = ny;
            *x = nx;
