...
fn main() {
    ...
    while action_cnt < MAX_TURN && !time_keeper.isTimeOver() {
        ...
        let (action, best_node) = beam.solve(
            width,
            depth.min(MAX_TURN - action_cnt),
            &mut input,
            &mut rng,
        );
    ...
    let mut i = 0;
    while i < actions.len() {
        let is_swap = actions[i];
        let dir1 = actions[i + 1];
        let dir2 = actions[i + 2];
        println!("{} {} {}", is_swap, DIRS[dir1], DIRS[dir2]);
        i += 3;
    }

    let score = (input.cost as f64).log2() - (init_node.score as f64).log2();
