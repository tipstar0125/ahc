...
impl State {
    ...
    fn add(&mut self, mut num: usize) {
        ...
        let current_len = self.hot_point.len();
        num = min!(num, L * L / 2 - current_len);
        while self.hot_point.len() < current_len + num {
            let y = rnd::gen_range(0, self.L);
            let x = rnd::gen_range(0, self.L);
            if self.hot_point.contains(&(y, x)) {
                continue;
            }
            self.P[y][x] = self.hot;
            self.hot_point.insert((y, x));
        }
        let mut dist = vec![];
        for i in 0..self.N {
            let (y0, x0) = self.YX[i];
            let mut d = vec![];
            for &(cy, cx) in self.hot_point.iter() {
                let dy = calc_diff(y0 as isize, cy as isize);
                let dx = calc_diff(x0 as isize, cx as isize);
                d.push((dy.abs() + dx.abs(), dy, dx));
            }
    ...
    fn remove(&mut self, mut num: usize) {
        let L = self.L;
        let calc_diff = |a0: isize, a1: isize| -> isize {
            let diff = a1 - a0;
            if diff.abs() <= L as isize / 2 {
        ...
        while self.hot_point.len() > current_len - num {
            ...
            self.P[y][x] = self.cold;
            self.hot_point.remove(&(y, x));
        }
        let mut dist = vec![];
        for i in 0..self.N {
            let (y0, x0) = self.YX[i];
...
impl Solver {
    fn solve(&mut self) {
        ...
        if S < 484 {
            ...
            let mut state = State::new(L, N, S, YX, delta, 1);
            ...
            let mut climbing_cnt = 0;

            while !time_keeper.isTimeOver() {
                climbing_cnt += 1;
                let mut next_state = state.clone();
                let hpn = next_state.hot_point.len();
                if rnd::gen_bool() {
                    if hpn >= L * L {
                        continue;
                    }
                    // let num = rnd::gen_range(1, L * L / 2);
                    let num = 1;
                    next_state.add(num);
                } else {
                    if hpn <= 1 {
                    ...
                    // let num = rnd::gen_range(1, L * L / 2);
                    let num = 1;
                    next_state.remove(num);
                }

                let score = f(&dummy_A, &dummy_F, &next_state, n);
                if score > score_max {
                    score_max = score;
                    state = next_state;
                }
            ...
            eprintln!("Climbing cnt: {}", climbing_cnt);
            eprintln!("delta: {}", delta);
            eprintln!("hot point num: {}", state.hot_point.len());
            ...
            'outer: for i in 0..N {
                ...
                for p in 0..state.hot_point.len() {
                    let mut pattern_out: BTreeMap<(isize, isize, isize), Vec<usize>> =
                        BTreeMap::new();
            ...
            let mut state = State2::new(L, N, S, YX, pattern_kind, delta);
...
fn f(A: &[usize], F: &[isize], state: &State, n: usize) -> usize {
    let N = state.N;
    let place_cost = state.calc_cost();
